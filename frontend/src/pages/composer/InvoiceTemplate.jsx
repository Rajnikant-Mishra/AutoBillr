...
// components/invoice/InvoiceTemplate.jsx  (or wherever it lives)
...
export default function InvoiceTemplate({
  ...
  // Normalize items so both formats work
    ...
    <div className="bg-surface p-10 w-full">
      ...
      <div className="flex justify-between mb-10">
        ...
        <div className="text-right">
          ...
          <p className="text-xs text-text-light uppercase mt-2">
            Issued · {invoice?.invoiceDate || invoice?.issueDate || "—"}
          </p>
        </div>
      </div>
        ...
        <tbody className="divide-y divide-border-light">
          {items.length === 0 ? (
            <tr>
              <td
                colSpan={4}
                className="py-6 text-center text-text-muted text-sm"
              >
                No items
              </td>
            </tr>
