...
const PLANS = [
  {
    id: "starter",
    name: "Starter",
    monthly: { price: 0, period: "Free" },
    yearly: { price: 0, period: "Free" },
    trial: "14 days",
    invoiceLimit: "10 invoices",
    popular: false,
    features: [
  ...
  },
  {
    id: "professional",
    name: "Professional",
    monthly: { price: 999, period: "/mo" },
    yearly: { price: 9990, period: "/year" },
    trial: "14 days",
    invoiceLimit: "Unlimited",
    popular: true,
    features: [
  ...
  },
  {
    id: "enterprise",
    name: "Enterprise",
    monthly: { price: 2499, period: "/mo" },
    yearly: { price: 24990, period: "/year" },
    trial: "14 days",
    invoiceLimit: "Unlimited",
    popular: false,
    features: [
...
const REGISTRATION_STORAGE_KEY = "autobillr-registration-draft";
const REGISTRATION_EMAIL_KEY = "autobillr-registration-email";
const REGISTRATION_PLAN_KEY = "autobillr-registration-plan";
const REGISTRATION_BILLING_KEY = "autobillr-registration-billing";
...
export default function Register() {
  ...
  // jobTitle = profile only (NOT workspace RBAC)
  // Workspace role is always "Owner" for the account creator
  const [formData, setFormData] = useState({
    firstName: "",
    lastName: "",
    email: "",
  ...
  // =====================================================
  // URL / SESSION PLAN + BILLING
  // =====================================================

  useEffect(() => {
    const planParam = searchParams.get("plan")?.trim().toLowerCase();
    const billingParam = searchParams.get("billing")?.trim().toLowerCase();

    const storedPlan = sessionStorage.getItem(REGISTRATION_PLAN_KEY);
    const storedBilling = sessionStorage.getItem(REGISTRATION_BILLING_KEY);

    if (planParam && VALID_PLAN_IDS.includes(planParam)) {
      setSelectedPlan(planParam);
      sessionStorage.setItem(REGISTRATION_PLAN_KEY, planParam);
    } else if (storedPlan && VALID_PLAN_IDS.includes(storedPlan)) {
      setSelectedPlan(storedPlan);
    }

    if (billingParam && VALID_BILLING_INTERVALS.includes(billingParam)) {
      setBillingInterval(billingParam);
      sessionStorage.setItem(REGISTRATION_BILLING_KEY, billingParam);
    } else if (
      storedBilling &&
      VALID_BILLING_INTERVALS.includes(storedBilling)
    ) {
      setBillingInterval(storedBilling);
    }
  }, [searchParams]);
  ...
  const handlePlanChange = (planId) => {
    if (!VALID_PLAN_IDS.includes(planId)) return;
    setSelectedPlan(planId);
    sessionStorage.setItem(REGISTRATION_PLAN_KEY, planId);
  };

  const handleBillingChange = (interval) => {
    if (!VALID_BILLING_INTERVALS.includes(interval)) return;
    setBillingInterval(interval);
    sessionStorage.setItem(REGISTRATION_BILLING_KEY, interval);
  };
  ...
  const checkEmailVerification = useCallback(
    async (email) => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            data.message || "Unable to check verification status"
          );
        }
  ...
  const startVerificationPolling = useCallback(
    (email) => {
      ...
      verificationPollingRef.current = setInterval(async () => {
        const verified = await checkEmailVerification(normalizedEmail);
        if (verified) stopVerificationPolling();
      }, 3000);
    },
    [checkEmailVerification, stopVerificationPolling]
  );
  ...
  useEffect(() => {
    ...
    let savedDraft = {};
    const storedDraft = sessionStorage.getItem(REGISTRATION_STORAGE_KEY);
    ...
    if (Object.keys(savedDraft).length > 0) {
      setFormData((prev) => ({
        ...prev,
        ...savedDraft,
        // migrate old "role" draft key → jobTitle
        jobTitle:
          savedDraft.jobTitle ||
          savedDraft.role ||
          prev.jobTitle,
        email: registrationEmail || savedDraft.email || prev.email,
      }));
    }
    ...
    if (registrationEmail) {
      ...
      sessionStorage.setItem(REGISTRATION_EMAIL_KEY, registrationEmail);
    }

    if (verifiedFromUrl && requestedStep === "2" && registrationEmail) {
      setEmailVerified(true);
      setEmailVerificationSent(false);
      stopVerificationPolling();
      setStep(2);
    ...
    if (registrationEmail && !verifiedFromUrl) {
      checkEmailVerification(registrationEmail);
    }
  }, [
    searchParams,
    navigate,
  ...
  const sendEmailVerification = async () => {
    ...
    try {
      ...
      setFormData(latestForm);
      saveDraft(latestForm);
      sessionStorage.setItem(REGISTRATION_EMAIL_KEY, normalizedEmail);
      ...
      try {
        ...
        throw new Error("Invalid server response from verification API");
      }
      ...
      showErrorToast(error.message || "Unable to send verification email");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleContinue = async () => {
    if (step === 1) {
      ...
      const alreadyVerified = await checkEmailVerification(formData.email);
      if (alreadyVerified) {
        stopVerificationPolling();
        setStep(2);
        return;
  ...
  const handleSubmit = async (e) => {
    ...
    if (!VALID_PLAN_IDS.includes(selectedPlan)) {
      showErrorToast("Please select a valid plan");
      setStep(3);
      return;
    }

    if (!VALID_BILLING_INTERVALS.includes(billingInterval)) {
      showErrorToast("Please select a valid billing cycle");
      setStep(3);
      return;
    }
    ...
    // Workspace RBAC role is ALWAYS Owner for the creator.
    // jobTitle is profile-only and must NOT be used as JWT role.
    const payload = {
    ...
    try {
      ...
      try {
        ...
        throw new Error("Invalid server response from registration API");
      }
      ...
      // Ensure token is a plain JWT string
      const token =
      ...
      setAuthData({
        ...
        subscription: data.subscription || data?.data?.subscription,
      });
      ...
      sessionStorage.removeItem(REGISTRATION_PLAN_KEY);
      sessionStorage.removeItem(REGISTRATION_BILLING_KEY);
            ...
            {[
              ...
              >
                ...
                <div className="flex items-center gap-2 text-xs text-slate-300">
                  ...
                  <span>
                    <b className="text-white">{t.name}</b> · {t.role}
                  </span>
                </div>
              </div>
            ))}
                      ...
                      <div className="mt-3 rounded-xl border border-primary/20 bg-primary-soft p-3">
                        <div className="flex items-start gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-text-muted mt-1 leading-relaxed">
                              We sent a verification link to{" "}
                              <strong>{formData.email}</strong>. Click the
                              link in the email.
                            </p>
                            <p className="text-xs text-primary font-semibold mt-2">
            ...
            {step === 2 && (
              <div>
                ...
                <div className="space-y-5">
                  ...
                  <div className="flex items-start gap-2.5 rounded-xl border border-primary/15 bg-primary-soft/50 p-3.5">
                    ...
                    <div>
                      ...
                      <p className="text-[12px] text-text-muted mt-0.5 leading-relaxed">
                        As Owner you get full access: invoices, clients,
                        projects, team, roles, and settings. You can invite
                        others later as Admin, Manager, Analyst, or Viewer.
                      </p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="Agency / Consulting">
                        Agency / Consulting
                      </option>
                ...
                <div className="flex justify-center mb-7">
                  ...
                  >
                    <button
                      ...
                      className={`
                        min-w-[110px] rounded-lg px-5 py-2.5 text-sm font-semibold transition-all
                        ${
                  ...
                  {PLANS.map((plan) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="mt-4 pt-4 border-t border-border">
                          <div className="flex items-center justify-between gap-2 text-[11px]">
                            <span className="text-text-muted">Free trial</span>
                            <span className="font-bold text-text">
                              {plan.trial}
                            </span>
                          </div>
            ...
            <div className="flex justify-between items-center mt-8">
              ...
              {step < 3 ? (
                <button
                  ...
                  disabled={loading}
                  className="
                    ml-auto px-6 py-3 bg-primary hover:bg-primary-hover text-white
                    rounded-xl font-semibold transition active:scale-[0.98]
                    shadow-sm shadow-primary/20 flex items-center gap-2
