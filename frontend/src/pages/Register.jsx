...
export default function Register() {
  ...
  // =====================================================
  // AUTO SELECT PLAN FROM URL (e.g. ?plan=starter)
  // =====================================================
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (verifiedFromUrl && requestedStep === "2" && registrationEmail) {
      ...
      stopVerificationPolling();
      setStep(2);
      saveDraft({ ...savedDraft, email: registrationEmail });
  ...
  const sendEmailVerification = async () => {
    ...
    try {
      setLoading(true);
      const latestForm = { ...formData, email: normalizedEmail };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!termsAccepted) {
      showErrorToast("Please accept the Terms of Service and Privacy Policy");
      return;
    }
      ...
      <main className="flex flex-1 flex-col md:flex-row">
        ...
        <section className="w-full md:w-1/2 lg:w-3/5 flex items-center justify-center p-8 md:p-16 bg-surface-secondary">
          <div className="w-full max-w-xl">
                        ...
                        <div className="flex items-start gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-text-muted mt-1 leading-relaxed">
                              ...
                              <strong>{formData.email}</strong>. Click the link in
                              the email.
                            </p>
            ...
            {/* STEP 3: CHOOSE PLAN (WITH INR ₹ & 14-DAY TRIAL) */}
            {step === 3 && (
              <div>
                <h2 className="text-2xl font-bold text-text mb-1">
                  Choose your plan
                ...
                <div className="bg-surface-secondary border border-border rounded-xl p-4 flex gap-3 items-start mt-6">
                  <span className="material-symbols-outlined text-primary">
                    card_giftcard
                  </span>
                  <div>
                    <div className="text-[13px] font-bold text-text">
                      14-Day Free Trial Included (₹0 Due Today)
                    </div>
                    <div className="text-[11.5px] text-text-muted mt-1 leading-relaxed">
                      No immediate payment required. Full access to all features. Cancel anytime during the trial.
                    </div>
                  </div>
                </div>
