...
export default function TeamPermissions() {
  ...
  const loadCustomRoles = useCallback(async () => {
    try {
      ...
      try {
        ...
        throw new Error("Invalid server response while loading roles.");
      }

      if (!res.ok || !result?.success) {
        throw new Error(result?.message || "Failed to load custom roles");
      }

      setCustomRoles(Array.isArray(result.data) ? result.data : []);
    } catch (error) {
  ...
  const totalRoles = 5 + customRoles.length;
  ...
  const handleMemberInvited = async (payload) => {
    try {
      ...
      if (invitedMember?.id) {
        setMembers((current) => {
          if (current.some((m) => m.id === invitedMember.id)) return current;
      ...
      console.error("Handle member invited error:", error);
            ...
            {[
              ...
              return (
                <button
                  ...
                  className={`
                    px-3.5 py-1.5 rounded-md text-[12.5px] font-semibold transition whitespace-nowrap
                    ${
                      active
                ...
                >
                  ...
                  {tab.count !== undefined && (
                    <span
                      className={`
                        ml-1.5 text-[10px] tabular-nums px-1.5 py-0.5 rounded-full font-bold
                        ${
                          active
        ...
        {activeTab === "Permissions" && (
          <div className="bg-surface rounded-xl shadow-sm border border-border-light p-6">
            ...
            <p className="text-sm text-text-muted mt-1">
              Configure what each role can access inside your AutoBillr workspace.
            </p>
            <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                ...
                <div
                  key={permission}
                  className="flex items-center justify-between p-4 rounded-lg bg-surface-secondary border border-border-light"
                >
