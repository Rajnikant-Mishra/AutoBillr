...
//     if (budget > 0 && billed > budget) return "risk";
...
//     if (statuses.some((s) => ["pending", "scheduled"].includes(s))) {
//       return "pending";
//     }
...
//         (row) => row.clientName || row.client?.name || "No Client",
...
//       }),
//     ],
//     [columnHelper, format, getProjectStatus]
//   );
...
//       await fetchProjects();
//       const createdProject = event?.detail?.project;
//       if (createdProject) notifyProjectCreated(createdProject, format);
...
//     return Math.max(0, ...projects.map((p) => Number(p?.budget || 0)));
//   }, [projects]);
...
//     setFilters((prev) => {
//       const currentMax = Number(prev?.budgetRange?.[1] || 0);
//       if (currentMax > 0 || maxBudgetAvailable === 0) return prev;
//       return { ...prev, budgetRange: [0, maxBudgetAvailable] };
//     });
...
//     if (activeFilter !== "all" && filters.projectStatus.length === 0) {
//       result = result.filter((p) => getProjectStatus(p) === activeFilter);
//     }

//     if (Array.isArray(filters.projectStatus) && filters.projectStatus.length > 0) {
//       result = result.filter((p) =>
//         filters.projectStatus.includes(getProjectStatus(p))
//       );
//     }
...
//     const minBudget = Number(filters.budgetRange?.[0] || 0);
//     const maxBudget =
//       filters.budgetRange?.[1] !== undefined && filters.budgetRange?.[1] !== null
//         ? Number(filters.budgetRange[1])
//         : Infinity;

//     if (maxBudget > 0 || maxBudgetAvailable === 0) {
//       result = result.filter((p) => {
...
//     if (filters.minBilled !== "" && filters.minBilled != null) {
//       const minBilled = Number(filters.minBilled);
//       if (!Number.isNaN(minBilled)) {
//         result = result.filter((p) => Number(p?.billed || 0) >= minBilled);
//       }
//     }

//     if (filters.maxBilled !== "" && filters.maxBilled != null) {
//       const maxBilled = Number(filters.maxBilled);
//       if (!Number.isNaN(maxBilled)) {
//         result = result.filter((p) => Number(p?.billed || 0) <= maxBilled);
//       }
//     }
...
//     return result;
//   }, [projects, activeFilter, filters, maxBudgetAvailable, getProjectStatus]);
...
//         if (milestone?.dueDate && status !== "paid") {
...
//     const projectList = Array.isArray(projects) ? projects : [];
//     return {
//       active: projectList.filter((p) => getProjectStatus(p) === "active").length,
//       pending: projectList.filter((p) => getProjectStatus(p) === "pending").length,
//       paid: projectList.filter((p) => getProjectStatus(p) === "paid").length,
//       risk: projectList.filter((p) => getProjectStatus(p) === "risk").length,
//     };
...
//       paid: projects.filter((p) => getProjectStatus(p) === "paid").length,
//       risk: projects.filter((p) => getProjectStatus(p) === "risk").length,
//     }),
//     [projects, getProjectStatus]
//   );
...
//       milestones.forEach((milestone) => {
//         if (milestone?.status === "paid" || !milestone?.dueDate) return;

//         const dueDate = new Date(milestone.dueDate);
//         if (Number.isNaN(dueDate.getTime()) || dueDate >= new Date()) return;

//         const notificationKey = `${project?.id}-${
//           milestone?.id || milestone?.index || milestone?.title
//         }`;

//         if (notifiedOverdueRef.current.has(notificationKey)) return;

//         notifiedOverdueRef.current.add(notificationKey);
//         notifyOverdue(project, milestone);
//       });
//     });
...
//     (milestoneIndex) => {
//       if (selectedProjectId === null || selectedProjectId === undefined) return;

//       setProjects((prev) =>
//         prev.map((project) => {
//           if (String(project?.id) !== String(selectedProjectId)) return project;
...
//               const currentIndex = MILESTONE_STATUSES.indexOf(currentStatus);
...
//       );
//     },
//     [selectedProjectId, notifyMilestonePaid, format]
//   );
...
//       if (editingMilestone && !editingMilestone.id && selectedProject) {
//         notifyMilestoneCreated(selectedProject, editingMilestone, format);
//       }
...
//                   const milestones = Array.isArray(selectedProject?.milestones)
...
export default function Projects() {
  ...
  const maxBudgetAvailable = useMemo(() => {
    ...
    return Math.max(
      0,
      ...projects.map((project) => Number(project?.budget || 0))
    );
  }, [projects]);
  ...
  useEffect(() => {
    ...
    setFilters((previousFilters) => {
      const currentMax = Number(previousFilters?.budgetRange?.[1] || 0);

      if (currentMax > 0 || maxBudgetAvailable === 0) {
        return previousFilters;
      }

      return {
        ...previousFilters,
        budgetRange: [0, maxBudgetAvailable],
      };
    });
  }, [projects, maxBudgetAvailable]);
  ...
  const filteredProjects = useMemo(() => {
    ...
    if (activeFilter !== "all" && filters.projectStatus.length === 0) {
      result = result.filter(
        (project) => getProjectStatus(project) === activeFilter
      );
    }
    ...
    if (
      ...
      result = result.filter((project) =>
        filters.projectStatus.includes(getProjectStatus(project))
      );
    }
    ...
    if (filters.minBilled !== "" && filters.minBilled != null) {
      ...
      if (!Number.isNaN(minBilled)) {
        result = result.filter(
          (project) => Number(project?.billed || 0) >= minBilled
        );
      }
    }
    ...
    if (filters.maxBilled !== "" && filters.maxBilled != null) {
      ...
      if (!Number.isNaN(maxBilled)) {
        result = result.filter(
          (project) => Number(project?.billed || 0) <= maxBilled
        );
      }
    }
  ...
  const statusCounts = useMemo(() => {
    ...
    return {
      ...
      paid: projectList.filter(
        (project) => getProjectStatus(project) === "paid"
      ).length,
      risk: projectList.filter(
        (project) => getProjectStatus(project) === "risk"
      ).length,
    };
  }, [projects, getProjectStatus]);
  ...
  useEffect(() => {
    ...
    projects.forEach((project) => {
      ...
      milestones.forEach((milestone) => {
        if (milestone?.status === "paid" || !milestone?.dueDate) {
          return;
        }
        ...
        if (notifiedOverdueRef.current.has(notificationKey)) {
          return;
        }
