...
export default function ProjectCard({
  title,
  client,           // can be object or string
  clientName,       // fallback
  endDate,
  dueDate,
  budget = 0,
  ...
  const getProjectStatus = () => {
    ...
    if (budgetAmount > 0 && billedAmount > budgetAmount) return "AT_RISK";

    if (!milestones || milestones.length === 0) return "ACTIVE";
    ...
    const hasPending = milestones.some((m) =>
      ["pending", "scheduled"].includes(String(m?.status || "").toLowerCase())
    );
  ...
  const formattedDueDate = (endDate || dueDate)
    ? new Date(endDate || dueDate).toLocaleDateString("en-US", {
        month: "short",
        day: "numeric",
        year: "numeric",
      })
      ...
      <div className="p-0">
          ...
          <div className="flex items-center gap-3 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-text-muted truncate">
                {displayClient} · {formattedDueDate}
              </div>
            </div>
          </div>
        ...
        <div className="space-y-2.5">
          ...
          <div className="h-2 bg-surface-secondary rounded-full overflow-hidden">
            <div
              className="h-full rounded-full bg-gradient-to-r from-primary to-info transition-all duration-slow"
          ...
          <div className="flex items-center justify-between text-[11px]">
            ...
            <span className="text-text-muted">{members || 0} members</span>
          </div>
        </div>
      </div>
    </Card>
