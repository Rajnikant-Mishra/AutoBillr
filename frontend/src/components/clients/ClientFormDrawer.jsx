...
const isDummyPhone = (phone) => {
  ...
  // All same digits like 0000000000 or 9999999999
    ...
    (targetStep = step) => {
      ...
      if (targetStep === 1) {
        ...
        if (isDummyEmail(contactEmail)) {
          showErrorToast("Dummy emails (@example, @test, etc.) are not allowed");
          return false;
        }
        if (phone && isDummyPhone(phone)) {
          showErrorToast("Please enter a valid phone number (dummy numbers not allowed)");
          return false;
        }
        if (formData.website.trim()) {
          const website = formData.website.trim();
